 ...
 * @author      XCQ
 * @package     user
 * @version     $Id: model.php 5005 2013-07-03 08:39:11Z 631753810@qq.com $
*/
?>
<?php
class userModel extends model
{
     ...
     * @return array
     */
    public function getPill($params = '', $usersToAppended = '')
    {
        if(defined('TUTORIAL')) return $this->loadModel('tutorial')->getUserPairs();
        /* Set the query fields and orderBy condition.
         *
        ...
        foreach($users as $account => $user)
        {
            $firstLetter = ucfirst(substr($account, 0, 1)) . ':';
            if(strpos($params, 'noletter') !== false) $firstLetter =  '';
            $users[$account] =  $firstLetter . (($user->deleted=== false) ? $account : (($user->deleted and $user->realname) ? "(离职)".$user->realname : $user->realname));
        }
     ...
     * @return array
     */
    public function getProjects1($account,$begin,$end)
    {
        $projects = $this->dao->select('t1.*,t2.*')->from(TABLE_TEAM)->alias('t1')
            ->leftJoin(TABLE_PROJECT)->alias('t2')->on('t1.root = t2.id')
            ->where('t1.type')->eq('project')
            ->andWhere('t2.openedDate')->ge($begin)
            ->andWhere('t2.openedDate')->le($end)
            ->andWhere('t1.account')->eq($account)
            ->andWhere('t2.deleted')->eq(0)
            ->orderBy('t2.id_desc')
            ->fetchAll();
