 ...
 * @author      XCQ
 * @package     testtask
 * @version     $Id: model.php 5114 2019-03-11 06:02:59Z 631753810@qq.com $
*/
?>
<?php
class testtaskModel extends model
{
     ...
     * @return  array
     */
    public function getByUser1($account,$begin,$end,$pager = null, $orderBy = 'id_desc', $type = '')
    {
        return $this->dao->select('t1.*, t2.name AS projectName, t3.name AS buildName')
            ->from(TABLE_TESTTASK)->alias('t1')
            ->leftJoin(TABLE_PROJECT)->alias('t2')->on('t1.project = t2.id')
            ->leftJoin(TABLE_BUILD)->alias('t3')->on('t1.build = t3.id')
            ->where('t1.deleted')->eq(0)
            ->andWhere('t1.begin')->ge($begin)
            ->andWhere('t1.end')->le($end)
            ->andWhere('t1.owner')->eq($account)
            ->beginIF($type == 'wait')->andWhere('t1.status')->ne('done')->fi()
            ->beginIF($type == 'done')->andWhere('t1.status')->eq('done')->fi()
            ->orderBy($orderBy)
