 ...
 * @author      XCQ
 * @package     story
 * @version     $Id: model.php 5145 2013-07-15 06:47:26Z 631753810@qq.com $
*/
?>
<?php
class storyModel extends model
{
     ...
     * @return array
     */
    public function getProjectStories2($projectID = 0, $orderBy = 't1.`order`_desc', $type = 'byModule', $param = 0, $pager = null)
    {
       
        if(defined('TUTORIAL')) return $this->loadModel('tutorial')->getProjectStories();
        $status=array('','draft','active','changed');
        if($type == 'bySearch')
        {
            $queryID  = (int)$param;
            $products = $this->loadModel('project')->getProducts($projectID);
     ...
     * @return array
     */
    public function getUserStories1($account, $begin,$end,$type = 'assignedTo', $orderBy = 'id_desc', $pager = null)
    {
        $stories = $this->dao->select('t1.*, t2.name as productTitle')->from(TABLE_STORY)->alias('t1')
            ->leftJoin(TABLE_PRODUCT)->alias('t2')->on('t1.product = t2.id')
            ->where('t1.deleted')->eq(0)
            ->andWhere('t1.openedDate')->ge($begin)
