 ...
 * @author      XCQ
 * @package     dashboard
 * @version     $Id: task.html.php 4771 2013-05-05 07:41:02Z 631753810@qq.com $
*/
?>
...
<div id='mainContent'>
  <nav id='contentNav'>
    <ul class='nav nav-default'>
      ...
      echo "<li class='$active'>" . html::a(inlink('task', "account=$account&begin=".strtotime($begin)."&end=".strtotime($end)."&type=assignedTo"), $lang->user->assignedTo) . "</li>";

      $active = $type == 'openedBy' ? 'active' : '';
      echo "<li class='$active'>" . html::a(inlink('task', "account=$account&begin=".strtotime($begin)."&end=".strtotime($end)."&type=openedBy"),   $lang->user->openedBy)   . "</li>";

      $active = $type == 'finishedBy' ? 'active' : '';
      echo "<li class='$active'>" . html::a(inlink('task', "account=$account&begin=".strtotime($begin)."&end=".strtotime($end)."&type=finishedBy"), $lang->user->finishedBy) . "</li>";

      $active = $type == 'closedBy' ? 'active' : '';
      echo "<li class='$active'>" . html::a(inlink('task', "account=$account&begin=".strtotime($begin)."&end=".strtotime($end)."&type=closedBy"),   $lang->user->closedBy)   . "</li>";

      $active = $type == 'canceledBy' ? 'active' : '';
      echo "<li class='$active'>" . html::a(inlink('task', "account=$account&begin=".strtotime($begin)."&end=".strtotime($end)."&type=canceledBy"), $lang->user->canceledBy) . "</li>";
      ?>
    </ul>
  </nav>
