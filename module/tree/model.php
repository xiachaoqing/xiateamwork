...
class treeModel extends model
{
    ...
    public function ReturnTree()
    {
        ...
        $array=$this->dao->select('id,name')->from(TABLE_MODULE)
            ->where('root')->eq(0)
     ...
     * product
     */
    public function ReturnPro($pro=0)
    {
        return $this->dao->select('t1.id,t1.name,t1.line')->from(TABLE_PRODUCT)->alias('t1')
        ->leftJoin(TABLE_MODULE)->alias('t2')->on('t2.id = t1.line')
            ->where('t1.status')->ne('closed')
            ->andWhere('t1.deleted')->eq(0)
            ->beginIF($pro)->andWhere('t1.line')->eq($pro)->fi()
    ...
    {
        return $this->dao->select('id,name,line')->from(TABLE_PRODUCT)
