...
<div id="mainMenu" class="clearfix">
  ...
  <?php
    ...
    if($config->global->flow == 'full')
    {
        ...
        common::printLink('report', 'todo', "account=$account&status=all", $label, '', "class='btn btn-link $active'");
    }
    ...
    if($config->global->flow != 'onlyTask' and $config->global->flow != 'onlyTest')
    {
        ...
        common::printLink('report', 'story', "account=$account", $label, '', "class='btn btn-link $active'");
    }
    ...
    if($config->global->flow == 'full' or $config->global->flow == 'onlyTask') 
    {
        ...
        common::printLink('report', 'task', "account=$account", $label, '', "class='btn btn-link $active'");
    }
    ...
    if($config->global->flow == 'full' or $config->global->flow == 'onlyTest') 
    {
        ...
        common::printLink('report', 'bug', "account=$account", $label, '', "class='btn btn-link $active'");

        $label  = "<span class='text'>{$lang->user->test}</span>";
        $active = ($methodName == 'testtask' or $methodName == 'testcase')? ' btn-active-text' : '';
        common::printLink('report', 'testtask', "account=$account", $label, '', "class='btn btn-link $active'");
    }
    ...
    if($config->global->flow == 'full' or $config->global->flow == 'onlyTask')
    {
        ...
        common::printLink('report', 'project',  "account=$account", $label, '', "class='btn btn-link $active'");
    }
    ?>
  </div>
</div>
